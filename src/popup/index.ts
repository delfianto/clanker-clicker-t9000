...
function readForm(): Settings {
  return {
    ...
    captchaSolver: {
      provider: ($<HTMLSelectElement>('captchaProvider')).value as Settings['captchaSolver']['provider'],
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  $<HTMLSelectElement>('captchaProvider').addEventListener('change', () => {
    $('apikey-row').classList.toggle('hidden', ($<HTMLSelectElement>('captchaProvider')).value === 'none');
  });
