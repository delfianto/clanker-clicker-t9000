...
const ONCLICK_SITES = [
  'financenube', 'mixrootmods', 'pastescript', 'trimorspacks',
  'aduzz', 'tutorialsaya', 'baristakesehatan', 'merekrut',
  'admediaflex', 'cdrab', 'financekita', 'jobydt',
  'cryptoinsiderhub', 'omnexa', 'bloggingos',
].map(d => `${d}\\.com`).join('|') + '|deltabtc\\.xyz|gadifeed\\.in';

const HREF_SITES = [
  'g34new', 'dlgamingvn', 'v34down', 'phimsubmoi', 'almontsf', 'fitmusclematrix',
  'amanguides', 'michaelemad', '7misr4day',
].map(d => `${d}\\.com`).join('|') + '|(nashib|timbertales)\\.xyz';
...
export const wpsafeRules: Rule[] = [
  ...
  },
  {
    ...
    match: [
      'admediaflex', 'cdrab', 'financekita', 'jobydt', 'foodxor', 'mealcold',
      'gkvstudy', 'thepragatishilclasses', 'indobo', 'pdfvale',
      'mkcgticket', 'techtunesbabu', 'mobilebajar',
    ].map(d => `${d}\\.com`).join('|') + '|gadifeed\\.in|skyfreecoins\\.top|vertohub\\.space',
    runAt: 'loaded',
    actions: [{
      type: 'wait-element',
